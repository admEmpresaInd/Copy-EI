# -*- coding: utf-8 -*-
"""
Gerador PPTX — Jubiabá com Brand System GSWP.
Lê _slides_parsed.json (89 slides) e gera jubiaba-gswp-brand.pptx.
"""
...
# python-pptx usa EMU. 1 px @ 96dpi = 9525 EMU.
PX = 9525
SLIDE_W = 1280 * PX
SLIDE_H = 720 * PX
...
# Dados financeiros para slides específicos
...
def diamond(slide, cx, y, color=GOLD):
    """Separador diamante decorativo centrado em cx."""
    size = 10*PX
...
def split_bullets(s):
    """Quebra texto com marcadores ● em lista; senão retorna [] e o texto inteiro."""
    s = clean(s)
    if '●' in s:
        # parte antes do primeiro ● é intro
        idx = s.find('●')
...
def layout_final(slide, d):
    ...
    for i,(val,lab) in enumerate(finals):
        x=sx+i*(cw+gap)
        add_rect(slide, x, sy, cw, 90*PX, STONE)
...
def layout_tickets(slide, d, chap_label):
    """Slide especial com os 3 tickets de investimento."""
    set_bg(slide, VOID)
    top_line(slide)
    overline(slide, chap_label)
...
def layout_content(slide, d, chap_label):
    ...
    if all_items:
        ...
        if len(all_items) > 4:
            ...
            add_bullets(slide, 80*PX, y, SLIDE_W-160*PX, SLIDE_H-y-50*PX, all_items, size=14)
    # referência
    ref = REFS.get(d['num'])
    if ref:
        txt(slide, 80*PX, SLIDE_H-52*PX, SLIDE_W-160*PX, 20*PX, "FONTE: "+ref,
...
# slides que são marcadores de capítulo/transição (título grande, pouco corpo)
CHAPTER_SLIDES = {'20','40','60','72','73','89'}  # encerramentos/transições
# slide de tickets
TICKET_SLIDE = '55'
...
def main():
    ...
    out = os.path.join(BASE,'jubiaba-gswp-brand.pptx')
    prs.save(out)
